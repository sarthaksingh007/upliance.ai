...
export const FieldEditor: React.FC = () => {
    ...
    const addOption = () => {
        const next = [...(field.options ?? []), { id: crypto.randomUUID(), label: 'New', value: `value${(field.options?.length ?? 0) + 1}`
} ];
dispatch(updateOptions({ id: field.id, options: next }));
};
                ...
                <Stack spacing={1}>
                    <TextField
                        ...
                        onChange={e => {
                            ...
                            dispatch(setDerived({ id: field.id, enabled: true, parents: options, expression: field.derived!.expression }));
                        }}
                    >
